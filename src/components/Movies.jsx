...
function Movies() {
  ...
  const search = (e) => {
    if (e.key === "Enter") {
      axios("https://www.omdbapi.com/?s=" + state.s + "&apikey=" + process.env.REACT_APP_API_KEY + "&type=movie").then(({ data }) => {
  ...
  useEffect(()=>{
    axios("https://www.omdbapi.com/?s=superman" + "&apikey=" + process.env.REACT_APP_API_KEY + "&type=movie").then(({ data }) => {
        let results = data.Search;
