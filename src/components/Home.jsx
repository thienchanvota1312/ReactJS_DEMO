...
function Home() {
  ...
  const search = (e) => {
    if (e.key === "Enter") {
      const url = 
      axios("https://www.omdbapi.com/?s=" + state.s + "&apikey=" + process.env.REACT_APP_API_KEY).then(({ data }) => {
  ...
  useEffect(()=>{
    axios("https://www.omdbapi.com/?s=super" + "&apikey=" + process.env.REACT_APP_API_KEY).then(({ data }) => {
        let results = data.Search;
  ...
  const openPopup = id => {
    axios("https://www.omdbapi.com/?i=" + id + "&apikey=" + process.env.REACT_APP_API_KEY).then(({ data }) => {
      let result = data;
